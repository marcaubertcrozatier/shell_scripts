...
#
#
export REFDIR=/home/jerusalem2/sf/cdimages/swf/builds/platform_359_reference
export BUILDDIR=/home/jerusalem2/sf/cdimages/swf/cdimages/build/359PLATFORM
export REF=/space/nsuser/p4client/cbs3/3.5.90/swf/css-apps
export P4CLIENT=359apps_zan
...
echo;echo;echo "#######################"
cd $REFDIR
old_ref=`ls -d cbs-css-platform-359-[0-9]*|tail -1`
cd $BUILDDIR
new_ref=`ls -d cbs-css-platform-359-[0-9]*|sort -n -t"-" -k 7,7|tail -1`
...
clnum=`$P4 change -i < /home/nsuser/shell/p4specif_359ref.in|awk '{print $2}'`
...
echo "Envoi du mail"
echo
subject="[SwF/Reference] 3.5.90 New PLATFORM reference: $num_ref"
sed -e "s/!NUM_REF!/$num_ref/" \
< /home/nsuser/shell/359ref.in > /home/nsuser/shell/359ref.txt
email -s "${subject}" $RECIP2 < /home/nsuser/shell/359ref.txt
