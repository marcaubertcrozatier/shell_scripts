...
#
#
export REFDIR=/home/jerusalem2/sf/cdimages/swf/builds/b2c_32TR2x_reference
export BUILDDIR=/home/jerusalem2/sf/cdimages/swf/cdimages/build/32TR2xAPPS/myshapeb2c
export REF=/space/nsuser/p4client/cbs3/3.2.TR2.x/swf/apps
export P4CLIENT=32tb2c_cha
export P4PORT=perforce:1666
export P4PASSWD=nsuser
export P4=/usr/local/bin/p4
#export RECIP="List-Self-Service-Dev@comverse.com"
export RECIP="Marc.Aubert@comverse.com,Mohamed.Inelhaj@comverse.com"
...
echo;echo;echo "#######################"
cd $REFDIR
old_ref=`ls -d cbs-css-consumer-*|tail -1`
cd $BUILDDIR
new_ref=`ls -d cbs-css-consumer-*|sort -t"-" -k 7,7|tail -1`
...
clnum=`$P4 change -i < /home/nsuser/shell/p4specif_32TR2xref.in|awk '{print $2}'`
...
echo "Envoi du mail"
echo
subject="[SwF/Reference] 32TR2x New CONSUMER reference: $num_ref"
sed -e "s/!NUM_REF!/$num_ref/" \
< /home/nsuser/shell/32tb2cref.in > /home/nsuser/shell/32tb2cref.txt
email -s "${subject}" $RECIP < /home/nsuser/shell/32tb2cref.txt
