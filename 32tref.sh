...
#
#
export REFDIR=/home/jerusalem2/sf/cdimages/swf/builds/platform_32TR2x_reference
export BUILDDIR=/home/jerusalem2/sf/cdimages/swf/cdimages/build/32TR2xPLATFORM
...
echo;echo;echo "#######################"
cd $REFDIR
old_ref=`ls -d cbs-css-platform-*|tail -1`
cd $BUILDDIR
new_ref=`ls -d cbs-css-platform-*|sort -t"-" -k 7,7|tail -1`
...
echo "Envoi du mail"
echo
subject="[SwF/Reference] 32TR2x New PLATFORM reference: $num_ref"
sed -e "s/!NUM_REF!/$num_ref/" \
< /home/nsuser/shell/32tref.in > /home/nsuser/shell/32tref.txt
email -s "${subject}" $RECIP < /home/nsuser/shell/32tref.txt
