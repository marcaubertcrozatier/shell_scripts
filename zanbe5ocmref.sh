...
#
#
export REFDIR=/home/jerusalem2/sf/cdimages/swf/builds/ocm_zanbe5_reference
export BUILDDIR=/home/jerusalem2/sf/cdimages/swf/cdimages/build/ZANBE5OCM
export REF=/space/nsuser/p4client/zanbe5/swf/css-apps
...
echo;echo;echo "#######################"
cd $REFDIR
old_ref=`ls -d cbs-css-ocm-zanbe5*|tail -1`
cd $BUILDDIR
new_ref=`ls -d cbs-css-ocm-zanbe5*|sort -n -t"-" -k 7,7|tail -1`
...
clnum=`$P4 change -i < /home/nsuser/shell/p4specif_zanbe5ref.in|awk '{print $2}'`
...
sed -e "s/!NUM_REF!/$num_ref/" \
    -e "s/!VERSION!/$version/" \
< /home/nsuser/shell/zanbe5ocmref.in > /home/nsuser/shell/zanbe5ocmref.txt
email -s "${subject}" $RECIP2 < /home/nsuser/shell/zanbe5ocmref.txt
