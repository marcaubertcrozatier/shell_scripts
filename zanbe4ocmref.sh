...
#
#
export REFDIR=/home/jerusalem2/sf/cdimages/swf/builds/ocm_zanbe4_reference
export BUILDDIR=/home/jerusalem2/sf/cdimages/swf/cdimages/build/ZANBE4OCM
export REF=/space/nsuser/p4client/zanbe4/swf/css-apps
export P4CLIENT=zanbe4apps_zan
...
echo;echo;echo "#######################"
cd $REFDIR
old_ref=`ls -d cbs-css-ocm-zanbe4*|tail -1`
cd $BUILDDIR
new_ref=`ls -d cbs-css-ocm-zanbe4*|sort -n -t"-" -k 7,7|tail -1`
...
clnum=`$P4 change -i < /home/nsuser/shell/p4specif_zanbe4ref.in|awk '{print $2}'`
...
sed -e "s/!NUM_REF!/$num_ref/" \
    -e "s/!VERSION!/$version/" \
< /home/nsuser/shell/zanbe4ocmref.in > /home/nsuser/shell/zanbe4ocmref.txt
email -s "${subject}" $RECIP2 < /home/nsuser/shell/zanbe4ocmref.txt
