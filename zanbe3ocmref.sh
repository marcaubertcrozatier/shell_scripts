...
#
#
export REFDIR=/home/jerusalem2/sf/cdimages/swf/builds/ocm_zanbe3_reference
export BUILDDIR=/home/jerusalem2/sf/cdimages/swf/cdimages/build/ZANBE3OCM
export REF=/space/nsuser/p4client/zanbe3/swf/css-apps
export P4CLIENT=zanbe3apps_zan
...
echo;echo;echo "#######################"
cd $REFDIR
old_ref=`ls -d cbs-css-ocm-zanbe3*|tail -1`
cd $BUILDDIR
new_ref=`ls -d cbs-css-ocm-zanbe3*|sort -n -t"-" -k 7,7|tail -1`
...
clnum=`$P4 change -i < /home/nsuser/shell/p4specif_zanbe3ref.in|awk '{print $2}'`
...
sed -e "s/!NUM_REF!/$num_ref/" \
    -e "s/!VERSION!/$version/" \
< /home/nsuser/shell/zanbe3ocmref.in > /home/nsuser/shell/zanbe3ocmref.txt
email -s "${subject}" $RECIP2 < /home/nsuser/shell/zanbe3ocmref.txt
