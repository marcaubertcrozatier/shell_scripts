...
P4PASSWD=nsuser
export P4PASSWD
BUILDS=/home/jerusalem2/sf/cdimages/swf/milestones/3.2.0-7.1
SRCROOT=//depot/c-one
#SRCROOT=//depot/ccbs
...
do
	...
	case $comp in
		platform) comp2=platform
			  buildrep=$ROOTBUILDREP/C-ONEPLATFORM;;
		ocm) comp2=ocm
		     buildrep=$ROOTBUILDREP/C-ONEOCM;;
		apps) comp2=consumer
		      buildrep=$ROOTBUILDREP/C-ONEAPPS/myshapeb2c;;
		ik) comp2=is
		    buildrep=$ROOTBUILDREP/C-ONEAPPS/ik;;
	esac
	cd $buildrep
	lastbuild=`\ls -d cbs-css-$comp2*|tail -1`
	cd $BUILDS
	lastrbuild=`\ls -d cbs-css-$comp2*`
	firstcl=`\ls -d cbs-css-$comp2*|awk -F"-" '{print $8}'`
