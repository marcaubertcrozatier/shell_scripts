...
#
#
export REFDIR=/home/jerusalem2/sf/cdimages/swf/builds/platform_mob_reference
export BUILDDIR=/home/jerusalem2/sf/cdimages/swf/cdimages/build/MOBPLATFORM
...
echo;echo;echo "#######################"
cd $REFDIR
old_ref=`ls -d cbs-css-platform-mob-[0-9]*|tail -1`
cd $BUILDDIR
new_ref=`ls -d cbs-css-platform-mob-[0-9]*|sort -n -t"-" -k 8,8|tail -1`
...
echo "Envoi du mail"
echo
subject="[SwF/Reference] 3.5.50 New PLATFORM reference: $num_ref"
sed -e "s/!NUM_REF!/$num_ref/" \
< /home/nsuser/shell/mobref.in > /home/nsuser/shell/mobref.txt
email -s "${subject}" $RECIP2 < /home/nsuser/shell/mobref.txt
