...
ROOTRBUILDREP=/home/jerusalem2/sf/cdimages/swf/milestones/3.0.0-DIT41-TR4.6
...
do
	...
	case $comp in
		platform) comp2=platform
			  ...
			  buildbrep=$ROOTBUILDREP/CBS3300PLA;;
		ocm) comp2=ocm
		     buildrep=$ROOTBUILDREP/CCBSOCM
		     buildbrep=$ROOTBUILDREP/CBS3300OCM;;
		apps) comp2=consumer
		      buildrep=$ROOTBUILDREP/CCBSAPPS/myshapeb2c
		      buildbrep=$ROOTBUILDREP/CBS3300APPS/myshapeb2c;;
		ik) comp2=is
		    buildrep=$ROOTBUILDREP/CCBSAPPS/ik
		    buildbrep=$ROOTBUILDREP/CBS3300APPS/ik;;
	esac
	cd $buildrep
	lastbuild=`\ls -d cbs-css-$comp2*|tail -1`
	cd $buildbrep
	lastbbuild=`\ls -d cbs-css-$comp2*|tail -1`
	cd $ROOTRBUILDREP
	firstcl=`\ls -d cbs-css-$comp2*|tail -1|awk -F"-" '{print $7}'`
	...
	$P4 changes $SRCBROOT/$comp/... |grep -v nsuser > $TMP/$bcomp.txt
	#
	while read line
	do
...
# 	echo "<B>Last build on main branch:</B> $lastbuild" >> $HOME/p4smail.txt
 	echo "</li>" >> $HOME/p4smail.txt
 	echo "<li>" >> $HOME/p4smail.txt
 	echo "<B>Last build on 3.0.0 branch:</B> $lastbbuild" >> $HOME/p4smail.txt
 	echo "</li>" >> $HOME/p4smail.txt
 	echo "<li>" >> $HOME/p4smail.txt
