...
#
#
export REFDIR=/home/jerusalem2/sf/cdimages/swf/builds/ocm_zanbe8_reference
export BUILDDIR=/home/jerusalem2/sf/cdimages/swf/cdimages/build/ZANBE8OCM
export REF=/space/nsuser/p4client/zanbe8/swf/css-apps
export P4CLIENT=zanbe8apps_zan
...
echo;echo;echo "#######################"
cd $REFDIR
old_ref=`ls -d cbs-css-ocm-zanbe8*|tail -1`
cd $BUILDDIR
new_ref=`ls -d cbs-css-ocm-zanbe8*|sort -n -t"-" -k 7,7|tail -1`
...
clnum=`$P4 change -i < /home/nsuser/shell/p4specif_zanbe8ref.in|awk '{print $2}'`
...
sed -e "s/!NUM_REF!/$num_ref/" \
    -e "s/!VERSION!/$version/" \
< /home/nsuser/shell/zanbe8ocmref.in > /home/nsuser/shell/zanbe8ocmref.txt
email -s "${subject}" $RECIP2 < /home/nsuser/shell/zanbe8ocmref.txt
