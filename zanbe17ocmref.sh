...
#
#
export REFDIR=/home/jerusalem2/sf/cdimages/swf/builds/ocm_zanbe17_reference
export BUILDDIR=/home/jerusalem2/sf/cdimages/swf/cdimages/build/ZANBE17OCM
export REF=/space/nsuser/p4client/zanbe17/swf/css-apps
export P4CLIENT=zanbe17apps_zan
...
echo;echo;echo "#######################"
cd $REFDIR
old_ref=`ls -d cbs-css-ocm-zanbe17*|tail -1`
cd $BUILDDIR
new_ref=`ls -d cbs-css-ocm-zanbe17*|sort -n -t"-" -k 6,6|tail -1`
num_ref=`echo $new_ref | awk -F"-" '{print $6}'`
...
clnum=`$P4 change -i < /home/nsuser/shell/p4specif_zanbe17ref.in|awk '{print $2}'`
...
echo "Envoi du mail"
echo
subject="[SwF/Reference] $version New OCM reference: $num_ref"
sed -e "s/!NUM_REF!/$num_ref/" \
    -e "s/!VERSION!/$version/" \
< /home/nsuser/shell/zanbe17ocmref.in > /home/nsuser/shell/zanbe17ocmref.txt
email -s "${subject}" $RECIP2 < /home/nsuser/shell/zanbe17ocmref.txt
