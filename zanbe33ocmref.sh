...
#
#
export REFDIR=/home/jerusalem2/sf/cdimages/swf/builds/ocm_zanbe33_reference
export BUILDDIR=/home/jerusalem2/sf/cdimages/swf/cdimages/build/ZANBE33OCM
export REF=/space/nsuser/p4client/zanbe33/swf/css-apps
export P4CLIENT=zanbe33apps_zan
...
echo;echo;echo "#######################"
cd $REFDIR
old_ref=`ls -d cbs-css-ocm-zanbe33*|tail -1`
cd $BUILDDIR
new_ref=`ls -d cbs-css-ocm-zanbe33*|sort -n -t"-" -k 6,6|tail -1`
num_ref=`echo $new_ref | awk -F"-" '{print $9}'`
...
clnum=`$P4 change -i < /home/nsuser/shell/p4specif_zanbe33ref.in|awk '{print $2}'`
...
sed -e "s/!NUM_REF!/$num_ref/" \
    -e "s/!VERSION!/$version/" \
< /home/nsuser/shell/zanbe33ocmref.in > /home/nsuser/shell/zanbe33ocmref.txt
email -s "${subject}" $RECIP2 < /home/nsuser/shell/zanbe33ocmref.txt
