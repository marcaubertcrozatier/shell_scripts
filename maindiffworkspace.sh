...
#############################
VERSION=3.5.90-8.4
OLDVERSION=3.5.90-8.3
NAME="cbs-css-apps-3.5.90-TC8.4-TC8.3"
#############################
...
HARDPATH="/space/nsuser/p4client/c-one/css-apps/"
...
#	lastbuild=`\ls -d cbs-css-$comp2*|tail -1`
	...
	lastcl=`find . -type d -name "cbs-css-$comp2*"|awk -F"/" '{print $2}'|awk -F"-" '{print $6}'`
	cd $BUILDS
...
#	firstcl=`\ls -d cbs-css-$comp2*|awk -F"-" '{print $7}'`
	firstcl=`find . -type d -name "cbs-css-$comp2*"|awk -F"/" '{print $2}'|awk -F"-" '{print $6}'`
	llastcl=`$P4 changes $SRCROOT/$complc/... |head -1|awk '{print $2}'`
	nbcl=`expr $llastcl - $firstcl`
	#
	if [ ! "${nbcl}" = "0" ]
...
do
	...
	do
		...
		if [ $cl -gt $firstcl -a $cl -le $lastcl ]
