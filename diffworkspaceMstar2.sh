...
#############################
VERSION=3.5.90-Mstar2.0-3.2
#OLDVERSION=3.5.50-Mstar1.50-4.0
OLDVERSION=3.5.90-Mstar2.0-3.1
NAME="cbs-css-apps-3.5.90-MSTAR2.0-TC3.2-3.5.90-MSTAR2.0-TC3.1"
#############################
...
SRCROOT=//depot/cm/cm_c_one_3.5.90_mstar2.0
...
#	firstcl=`\ls -d cbs-css-$comp2*|awk -F"-" '{print $7}'`
	typeset -i firstcl=`find . -type d -name "cbs-css-$comp2*"|awk -F"/" '{print $2}'|awk -F"-" '{print $7}'`
	typeset -i lastcl=`$P4 changes $SRCROOT/$complc/... |head -1|awk '{print $2}'`
typeset -i lastcl=122636
typeset -i firstcl=122179
