...
SUBJECT="Clearquest POS Bugs status"
...
cd $HOME
if [ -f bposmail.txt ]
then rm -f bposmail.txt
fi
if [ -f .lastbposbug.txt ]
then rm -f .lastbposbug.txt
fi
touch .lastbposbug.txt
sed -e "s/!BUILD!/$BUILDNAME/"< bposmail.in > bposmail.txt
#
#
# PR DEFECT_TYPE
#
#
##############################
sqlplus CQ_RO/Comverse@cqprdcbs @bpos "/home/oracle/oracle/product/10.2.0/db_2/oradata/cqprdcbs"|grep -v "All Rights Reserved." |grep -v "Connected to:"|grep -v "With the Partitioning, OLAP and Data Mining options"|grep -v "CBS_MODULE"|grep -v '\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-'|grep -v SEVERITY|grep -v PRIORITY|grep -v ID|grep -v "TITLE"|grep -v SERVICE_NAME |grep -v STATE |grep -v SUBMIT_DA|grep -v ASSIGN_TO_FULLNAME|grep -v CUSTOMER_NAME|grep -v DBID|grep -v "Release 10.2.0.3.0"|grep -v NAME >bpos.txt
##############################
echo "<BR><BR>" >> $HOME/bposmail.txt
echo "<H2><font color=0080ff>ClearQuest bpos PR status:</font></H2>" >> $HOME/bposmail.txt
echo "<TABLE BORDER=1 CELLPADDING=4>" >> $HOME/bposmail.txt
echo "<TR  BGCOLOR=DDDDDD>" >> $HOME/bposmail.txt
echo "<TH>#</TH>" >> $HOME/bposmail.txt
echo "<TH>Customer</TH>" >> $HOME/bposmail.txt
echo "<TH>Service</TH>" >> $HOME/bposmail.txt
echo "<TH>Module</TH>" >> $HOME/bposmail.txt
echo "<TH>ID</TH>" >> $HOME/bposmail.txt
echo "<TH>Title</TH>" >> $HOME/bposmail.txt
echo "<TH>Severity</TH>" >> $HOME/bposmail.txt
echo "<TH>Priority</TH>" >> $HOME/bposmail.txt
echo "<TH>Assign To</TH>" >> $HOME/bposmail.txt
echo "<TH>State</TH>" >> $HOME/bposmail.txt
echo "<TH>Submit Date</TH>" >> $HOME/bposmail.txt
echo "</TR>" >> $HOME/bposmail.txt
i=7
nb=1
LAST=0
while read line
  do
                ...
                then
                        ...
                        then case $line in
                                CSS*|MyNetonomy* )i=7
						  owner=""
						  echo "<TR BGCOLOR=F6F6F6><TD>$nb</TD><TD>$customer</TD><TD><CENTER>$sname1 $sname2</CENTER></TD><TD>$module</TD><TD>$id</TD><TD>$title</TD><TD>$severity</TD><TD BGCOLOR=$bgcolor>$priority</TD><TD>$owner</TD><TD>$state</TD><TD>$sdate</TD></TR>" >> $HOME/bposmail.txt
                             			  echo $id >> $HOME/.lastbposbug.txt
                             			  nb=`expr $nb + 1`;;
                                *)i=1;;
                             esac
                        fi
...
#			esac
			     ...
			     link=`echo $LINK|sed -e "s/!DBID!/$dbid/"`
                             echo "<TR BGCOLOR=F6F6F6><TD>$nb</TD><TD>$customer</TD><TD><CENTER>$sname1 $sname2</CENTER></TD><TD>$module</TD><TD><a href=$link>$id</a></TD><TD>$title</TD><TD>$severity</TD><TD BGCOLOR=$bgcolor>$priority</TD><TD>$owner</TD><TD>$state</TD><TD>$sdate</TD></TR>" >> $HOME/bposmail.txt
			     echo $id >> $HOME/.lastbposbug.txt
                             nb=`expr $nb + 1`
                        fi
                        if [ $i -lt 0 ]
...
style='font-size:12.0pt'><o:p>&nbsp;</o:p></span></font></p>" >> $HOME/bposmail.txt

#
echo "</BODY></HTML>" >> $HOME/bposmail.txt
email -html -s "$SUBJECT" $RECIP < $HOME/bposmail.txt
