...
#
#
export REFDIR=/home/jerusalem2/sf/cdimages/swf/builds/ocm_zanbe6_reference
export BUILDDIR=/home/jerusalem2/sf/cdimages/swf/cdimages/build/ZANBE6OCM
export REF=/space/nsuser/p4client/zanbe6/swf/css-apps
export P4CLIENT=zanbe6apps_zan
...
echo;echo;echo "#######################"
cd $REFDIR
old_ref=`ls -d cbs-css-ocm-zanbe6*|tail -1`
cd $BUILDDIR
new_ref=`ls -d cbs-css-ocm-zanbe6*|sort -n -t"-" -k 6,6|tail -1`
...
clnum=`$P4 change -i < /home/nsuser/shell/p4specif_zanbe6ref.in|awk '{print $2}'`
...
sed -e "s/!NUM_REF!/$num_ref/" \
    -e "s/!VERSION!/$version/" \
< /home/nsuser/shell/zanbe6ocmref.in > /home/nsuser/shell/zanbe6ocmref.txt
email -s "${subject}" $RECIP2 < /home/nsuser/shell/zanbe6ocmref.txt
